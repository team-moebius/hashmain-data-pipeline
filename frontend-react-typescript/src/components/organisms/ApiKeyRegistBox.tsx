...
class ApiKeyRegistBox extends React.Component<ApiKeyRegistBoxProps, ApiKeyRegistBoxState> {
  ...
  isValid = () => {
    ...
    return (
      InputValidator.isBlank(this.accessKeyRef.current.value) &&
      InputValidator.isBlank(this.secretKeyRef.current.value) &&
      InputValidator.isBlank(this.botNameRef.current.value)
    );
  };
    ...
    return (
      <form onSubmit={this.onSubmit}>
        ...
        <ul>
          ...
          <li>
            <Text gutterBottom variant="caption">
              * 매매 포지션을 설정하는 것은 전적으로 회원의 선택에 따른 것으로서 가상화폐 시세 하락으로 거래 결과가
              손해로 이어질 수 있으며, ICORE. Inc는 이에 대하여 책임을 지지 않습니다.
            </Text>
          </li>
        </ul>
