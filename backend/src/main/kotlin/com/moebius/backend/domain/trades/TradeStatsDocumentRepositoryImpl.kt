...
class TradeStatsDocumentRepositoryImpl(@Autowired override val client: RestHighLevelClient) :
    ...
    override fun generateTradeStats(startDateTime: LocalDateTime, interval: ElasticUtils.AggregationInterval): List<TradeStatsDocument> {
        return TradeStatsGenerator(startDateTime, interval).generate(client)
    }

    private class TradeStatsGenerator(
        ...
        private fun deserializeResponse(response: SearchResponse): List<TradeStatsDocument> {
            ...
            println("range total doc count: ${filter.docCount}")
        ...
        private fun parseHistogram(aggDate: Histogram.Bucket): List<TradeStatsDocument> {
            println("date: ${aggDate.keyAsString} (${aggDate.key}) doc-count: ${aggDate.docCount}")
        ...
        private fun parseExchange(aggExchange: Terms.Bucket): List<TradeStatsDocument.Builder> {
            println("exchange: ${aggExchange.keyAsString} doc-count: ${aggExchange.docCount}")
        ...
        private fun parseTradeTypeAndSum(aggTradeType: Terms.Bucket): Triple<TradeType, Long, Double> {
            println("tradeType: ${aggTradeType.keyAsString} doc-count: ${aggTradeType.docCount}")
